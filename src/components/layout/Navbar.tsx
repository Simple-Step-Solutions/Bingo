...
export const Navbar: React.FC<NavbarProps> = ({ user, settings }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-xl border-t border-neutral-200 px-6 py-4 flex justify-around items-center z-40 md:top-0 md:bottom-auto md:border-t-0 md:border-b shadow-2xl md:shadow-sm">
      ...
      <button 
        onClick={() => signOut(auth)}
        className="md:hidden flex flex-col items-center gap-1 text-neutral-400 hover:text-red-600 transition-colors"
      >
        <div className="p-2">
          <LogOut size={20} />
        </div>
